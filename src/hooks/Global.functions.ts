...
const convertComponentRestaurant = (
  restaurants: ComponentCommonRestaurant,
  avalaible = false
): PriceRestaurant[] => {
  return (
...
const convertComponentIngredient = (
  ingredientRelationColleciton: IngredientEntity[]
): string[] => {
  return (
    ingredientRelationColleciton?.reduce(
      (acc, ingredientValue) =>
        ingredientValue.id ? [...acc, ingredientValue.id] : acc,
      [] as string[]
    ) ?? []
  );
};
...
export const convertRestaurantApp = (
  restaurantCollection: RestaurantsQuery
): RestaurantApp[] => {
    ...
    restaurants = restaurantCollection.data.map(({ id, attributes }) => {
      ...
      const schedule = attributes?.schedule
        ? attributes?.schedule?.reduce((acc, currentValue) => {
            const days =
              currentValue?.days?.data?.reduce(
                (accDays, { attributes: attributesDays }) => {
                  return attributesDays
                    ? [
                        ...accDays,
                        { key: attributesDays.key, day: attributesDays.day },
                      ]
                    : accDays;
                },
  ...
  if (saladCollection) {
    salads = saladCollection.data.map(({ id, attributes }) => {
      const restaurant = attributes?.restaurant
        ? convertComponentRestaurant(
            attributes.restaurant as ComponentCommonRestaurant
          )
        : [];
  ...
  if (sideCollection) {
    sides = sideCollection.data.map(({ id, attributes }) => {
      ...
      const ingredients = attributes?.ingredients?.data
        ? convertComponentIngredient(attributes.ingredients.data)
        : [];
      const sauces = attributes?.sauces?.data
        ? convertComponentIngredient(attributes.sauces.data)
        : [];
...
export const convertSandwichApp = (
  sandwichCollection: SandwichQuery
): SandwichApp[] => {
...
export const convertBurgerApp = (
  burgerCollection: BurgerQuery
): BurgerApp[] => {
...
export const convertOptionApp = (
  optionCollection: OptionQuery
): OptionApp[] => {
...
export function convertSimpleModel(
  collection: BeveragesQuery | DessertsQuery
): (BeverageApp | DessertApp)[] {
...
export const convertCategoryApp = (
  collectionCategory: CategoryQuery
): CategoryApp[] => {
...
export const convertIngredientApp = (
  ingredientCollection: IngredientQuery
): IngredientApp[] => {
  ...
  if (ingredientCollection) {
    ingredients = ingredientCollection.data.map(({ id, attributes }) => {
      const restaurant = attributes?.restaurants
        ? convertComponentRestaurant(
            attributes.restaurants as ComponentCommonRestaurant,
            true
          )
        : [];
