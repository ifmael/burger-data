...
const getOptions = (
  options: OptionApp[],
  optionId: string
): OptionApp | undefined => options.find(({ id }) => id === optionId);
...
function getPriceRestaurant(
  priceRestaurant: PriceRestaurant[],
  idRestaurant: string
): PriceRestaurant | undefined;
function getPriceRestaurant(
  priceRestaurant:
    | PriceRestaurant[]
  ...
  | (PriceRestaurant & BurgerConfiguration)
  | undefined {
  return priceRestaurant.find(
    (itemRestaurant) => itemRestaurant.restaurantId === idRestaurant
  );
}
...
export const convertItem = (items: ItemApp[], idRestaurant: string): Item[] => {
  return items.reduce((acc, item) => {
    ...
    return resturantItem
      ? [...acc, { id: item.id, name: item.name, price: resturantItem.price }]
      : acc;
  }, [] as Item[]);
};
...
export const convertIngredient = (
  ...
  return ingredients.reduce((acc, ingredient) => {
    const resturantItem = getPriceRestaurant(
...
export const convertSidePage = (
  ...
  return sides.reduce((acc, side) => {
    ...
    return resturantItem
      ? [...acc, { ...sidePage, price: resturantItem.price }]
      : acc;
  }, [] as SidePage[]);
};

export const convertSaladPage = (
  salads: SaladApp[],
  idRestaurant: string,
  ingredients: Ingredient[]
): SaladPage[] => {
  return salads.reduce((acc, saladInner) => {
    ...
    return resturantItem
      ? [...acc, { ...saladPage, price: resturantItem.price }]
      : acc;
  }, [] as SaladPage[]);
};
...
export const convertBurgerPage = (
  ...
  return burgers.reduce((acc, currentBurger) => {
    const resturantItem = getPriceRestaurant(
      currentBurger.restaurants,
      idRestaurant
    );
    const ingredientsProduct = getElements(
    ...
    if (resturantItem) {
      ...
      const meatPointOption = getOptions(
        options,
        resturantItem.meatPointOption
      );
      ...
      const bread = breadOption?.items
        ? getElements(breadOption.items, items)
        : [];
      const beveragesProduct: Beverage[] = beverageOption?.beverages
        ? getElements(beverageOption.beverages, beverages)
        : [];
      const sidesPage = sideOption?.sides
        ? getElements(sideOption?.sides, sides)
        : [];
      const meat = meatOption?.items
        ? getElements(meatOption.items, items)
        : [];
      const meatPoint = meatPointOption?.items
        ? getElements(meatPointOption.items, items)
        : [];
...
export function convertSimpleModel(
  ...
  return elements
    .map((el) => {
      ...
      return resturantItem
        ? { id: el.id, name: el.name, price: resturantItem.price }
        : null;
    })
